...
└── test/
    ...
    ├── test_phase4.jl         ✅  Validation — edge cases, invariants, regression
    ├── reference_data/        ✅  CSV reference outputs (generated by scripts below)
    ├── generate_reference_data.jl       ✅  Generate CSVs from Julia solver
    ├── generate_reference_data_matlab.m ✅  Generate CSVs from MATLAB mfeval (external)
    └── benchmarks.jl          ✅  BenchmarkTools suite — scalar, alloc, type-stability, thread scaling
```
...
============================================================
Test Summary:          | Pass  Total  Time
Phase 2: calc_inst_Kya |    2      2  0.0s
... (97 tests total)
============================================================
Phase 3 — Public API
============================================================
Test Summary:                                 | Pass  Total  Time
Phase 3: batch input widths (6, 7, 8 columns) |    3      3  0.1s
... (55 tests total)
============================================================
Phase 4 — Validation
============================================================
Test Summary:                                       | Pass  Total  Time
Phase 4: batch vs scalar — kappa x alpha grid (5x5) |   25     25  0.0s
... (117 tests total)
```

**611 tests, 0 failures** across all four phases.
...
**Test file:** `test/test_phase4.jl`

Four test groups, all wired into `runtests.jl`:

| Group | What it checks |
|---|---|
| **Regression** | Re-evaluate CSV reference inputs; assert every output column within `tol < 1e-6` |
| **Edge cases** | `Fz=0`, `Vx=0`, locked wheel (`κ=−1`), max camber, inputs beyond valid range, low-speed regime |
| **Physical invariants** | ISO-W sign conventions, geometry bounds (`Rl < R0`, `ρ > 0`), all three MF versions |
| **Batch vs scalar** | Numerical identity between `mfeval(scalar)` and `mfeval(batch)` for full `Fz×α` and `κ×α` grids |

**Reference data workflow:**

```bash
# Step 1 — Generate CSVs from Julia (self-consistent, available now)
julia --project=. test/generate_reference_data.jl

# Step 2 (optional) — Regenerate from MATLAB for true external validation
#   requires MATLAB + mfeval toolbox on the path
matlab -batch "run('test/generate_reference_data_matlab.m')"

# Step 3 — Run Phase 4 tests (regression tests auto-skip if no CSVs)
julia --project=. test/runtests.jl
```

Regression testsets use `@test_skip` when the reference CSV is absent, so
`runtests.jl` always exits cleanly — even before reference data is generated.

### Phase 5 — Performance ✅

**Benchmark file:** `test/benchmarks.jl`

Run separately (not included in `runtests.jl` — benchmarks are not a correctness gate):

```bash
# Single-threaded
julia --project=. test/benchmarks.jl

# Multi-threaded (to measure thread scaling)
julia --project=. --threads=4 test/benchmarks.jl
```

Five benchmark sections:

| Section | What it measures | Pass criterion |
|---|---|---|
| **1 · Scalar path** | Median time per evaluation, all three MF versions | `< 1 µs` |
| **2 · Allocation audit** | Heap allocations on the scalar hot path | `0 allocs` |
| **3 · Type stability** | `@code_warntype` scan for `Any` / `Union{}` | Zero instabilities |
| **4 · Batch throughput** | ns/eval for N = 100 / 1 000 / 10 000 rows | `< 1 µs/eval` |
| **5 · Thread scaling** | Parallel speedup vs ideal serial baseline | `≥ 50 % efficiency` |

All sections print colour-coded `PASS` / `WARN` / `FAIL` lines.  No Julia process
restarts or special flags are needed beyond `--threads=N` for section 5.

**Measured results on Apple M1 (8 threads):**

```
1 · Scalar path
  MF6.1  combined slip + camber    864 ns  ✅
  MF6.2  (iterative Rl solver)    1225 ns  ⚠️  (+400 ns from secant Rl loop — expected)
  MF5.2                            740 ns  ✅
  MF6.1  turn-slip (M112)         761 ns  ✅

2 · Allocation audit
  All three MF versions:  0 allocations  ✅

3 · Type stability
  All three MF versions:  zero Any / Union{}  ✅

4 · Batch throughput  (8 threads)
  N=   100   1371 ns/eval   ⚠️  (thread-pool overhead dominates at small N)
  N=  1000    313 ns/eval   ✅
  N= 10000    191 ns/eval   ✅

5 · Thread scaling  (8 threads, N=10 000)
  Parallel speedup:   4.50×
  Thread efficiency:  56%  ✅  (≥ 50% target)
```
